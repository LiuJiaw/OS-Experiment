...
void ShowInfo()
{
	cout<<"LRU的缺页次数为:"<<LRU::getInstance()->gettimes()<<"\t缺页率为"<<(float)LRU::getInstance()->gettimes()/TIME<<endl;
	cout<<"FIFO的缺页次数为:"<<FIFO::getInstance()->gettimes()<<"\t缺页率为"<<(float)FIFO::getInstance()->gettimes()/TIME<<endl;
}
